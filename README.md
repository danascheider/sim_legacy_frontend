...
On successful authorisation, the Google response returns a [JWT bearer token](https://jwt.io/introduction). In SIM's login success callback, a request is made to the API's `/auth/verify_token` endpoing with the token in the `Authorization` header. This endpoint verifies the token on the server side and creates a user profile in the database populated with data from Google (user profiles cannot be directly created or updated through the API). When the request succeeds, the `_sim_google_session` cookie is added with the JWT bearer token as the value and the user is redirected to the `/dashboard` page. (User profiles cannot be created or updated directly through the API.) Once the server has verified the user, they are redirected to their dashboard.
